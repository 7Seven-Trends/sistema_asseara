...
class NoticiasController extends Controller
{
    ...
    public function consultar(Request $request){
        if($request->isMethod('get')){
            ...
            return view("noticias.consultar", ["noticias" => $noticias]);
        }else{
            $filtros = [];
    ...
    public function cadastrar(Request $request){
        ...
        if($request->file("preview")){
            $noticia->preview = asset($request->file('preview')->store(
                'site/imagens/noticias/' . Str::slug($noticia->titulo), 'local'
            ));
        }

        if($request->file("banner")){
            $noticia->banner = asset($request->file('banner')->store(
                'site/imagens/noticias/' . Str::slug($noticia->titulo), 'local'
            ));
        }
    ...
    public function preview(Noticia $noticia){
        ...
        return view("noticias.preview", ["noticia" => $noticia, "configuracoes" => $configuracoes]);
    }

    public function visitas(Noticia $noticia){
        return view("noticias.leads", ['noticia' => $noticia]);
    }
}
